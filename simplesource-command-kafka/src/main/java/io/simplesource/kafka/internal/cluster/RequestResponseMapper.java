...
class RequestResponseMapper {
    ...
    private static final Result<CommandError, NonEmptyList<Sequence>> REMOTE_LOOKUP_TIMED_OUT =
            Result.failure(CommandError.of(Reason.Timeout, "Remote lookup timed out"));

    private final ScheduledExecutorService scheduler;
    private final AtomicLong idGenerator = new AtomicLong(0);
    private final Map<Long, CompletableFuture<Result<CommandError, NonEmptyList<Sequence>>>> requestMap = new ConcurrentHashMap<>();
    ...
    MappedCommandRequest newCommandRequest(HostInfo targetHost, String aggregateName, UUID commandUUId, Duration timeout) {
        ...
        final CompletableFuture<Result<CommandError, NonEmptyList<Sequence>>> cf = new CompletableFuture<>();
    ...
    private void completeRequest(Message.CommandResponse commandResponse, boolean scheduled) {
        CompletableFuture<Result<CommandError, NonEmptyList<Sequence>>> future = requestMap.remove(commandResponse.requestId);
        if (future != null) {
