...
import static io.simplesource.kafka.internal.streams.topologies.TestAggregateBuilder.stateStoreName;
...
class AggregateUpdatePublisherTest {
    ...
    private AggregateUpdatePublisher<String, TestCommand, TestEvent, Optional<TestAggregate>> target;
    ...
    void setUp() {
        ...
        topologyTestDriver = new TopologyTestDriverInitializer()
                ...
                .<String, AggregateUpdateResult<Optional<TestAggregate>>>build(s -> {
                    target.toAggregateStore(s);
                    target.toCommandResultStore(s);
                    target.toCommandResponseTopic(s);
                });
